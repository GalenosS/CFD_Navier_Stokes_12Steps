...
# # Trace de la surface
# # rstride/cstride : Sauter des lignes pour alleger le rendu graphique
# surf = ax.plot_surface(X, Y, u, cmap=cm.viridis, rstride=2, cstride=2, linewidth=0, antialiased=False)

# # Labels et Titres
# ax.set_title(f'Burgers 2D (t={nt}, nu={nu})\nConvection + Diffusion couplees')
# ax.set_xlabel('Axe X')
# ax.set_ylabel('Axe Y')
# ax.set_zlabel('Vitesse U')
...
# print(f"equilibre atteint en {iterations} iterations.")
...
# ax.set_title(f'equation de Laplace (equilibre de Pression)\nConverge en {iterations} iterations')
# ax.set_xlabel('X')
# ax.set_ylabel('Y')
...
# """
# # On lance le calcul
# # On fait 100 iterations pour laisser la pression s'etablir
...
# ax.set_title('equation de Poisson (Source + Puits)')
# ax.set_xlabel('X')
# ax.set_ylabel('Y')
# ax.set_zlabel('Pression P')
# ax.view_init(elev=30, azim=220)
...
"""
Equation de Navier-Stokes en cavite ,en 2D.
"""

# --- Simulation ---
print("Demarrage de Navier-Stokes (Cavity Flow)...")
# On lance 700 pas de temps pour que le tourbillon s'installe bien
# nit=50 assure que la pression est bien calculee a chaque etape
...
# alpha=0.5 rend la couleur un peu transparente pour voir les fleches
contour = plt.contourf(X, Y, p, alpha=0.5, cmap=cm.viridis)
plt.colorbar(contour, label='Pression')

# 2. On affiche le champ de vitesse (Quiver / Fleches)
# On saute quelques points (skip) pour ne pas surcharger le graphique
skip = 2
plt.quiver(X[::skip, ::skip], Y[::skip, ::skip], 
           u[::skip, ::skip], v[::skip, ::skip])

plt.title('Navier-Stokes : Cavite Entraînee\n(Fleches = Vitesse, Couleurs = Pression)')
plt.xlabel('X')
plt.ylabel('Y')

# On force un aspect carre
plt.axis('scaled')

if not os.path.exists("../results"):
    os.makedirs("../results")
plt.savefig("../results/final_cavity_flow.png", dpi=300) # Sauvegarde en HD
print("Image sauvegardee dans results/final_cavity_flow.png")
...
# --- Simulation ---
print("Demarrage de Navier-Stokes (Channel Flow)...")
# On prend un nu un peu plus fort pour bien voir la couche limite se developper
...
# 1. Champ de Pression (Fond colore) + Vitesse (Fleches)
# La pression doit diminuer de gauche (haute) a droite (basse) pour pousser le fluide
contour = plt.contourf(X, Y, p, alpha=0.5, cmap=cm.viridis)
plt.colorbar(contour, label='Pression')
...
plt.title('Channel Flow : Pression (Couleurs) et Vitesse (Fleches)')
...
# 2. Ajoutons un graphique "Preuve" : Le profil de vitesse a la sortie
# On prend une coupe verticale a la fin du canal (derniere colonne)
plt.figure(figsize=(6, 4))

# Donnees simulees
plt.plot(u[:, -1], Y[:, -1], 'r-o', label='Simulation (Sortie)')

# Donnees theoriques (Profil de Poiseuille)
# Pour un ecoulement entre plaques planes, Umax = 1.5 * Umoy
# Umoy = 1 (vitesse d'entree imposee) -> Umax = 1.5
# H = 2.0 (hauteur du canal)
# Formule : u(y) = 4 * Umax * (y/H) * (1 - y/H)
H = 2.0
u_max_theorique = 1.5 * 1.0 # 1.0 est la vitesse d'entree
y_plot = Y[:, -1]
u_theorique = 4 * u_max_theorique * (y_plot / H) * (1 - y_plot / H)

plt.plot(u_theorique, y_plot, 'k--', linewidth=2, label='Theorie (Poiseuille)')

plt.title('Validation : Profil de vitesse vertical vs Theorie')
